...
- **Silent registration conflict.** If the OS or another app has already claimed the combo, soffes/HotKey installs the hotkey but the callback never fires; no error is returned. v1 mitigation: pick uncommon combos (Option+F13 is the v1 default). v2 plan: surface conflicts via `CGEventSource` introspection at registration time. Symptom: hotkey "doesn't work" with no log line — verify by trying a different combo.
