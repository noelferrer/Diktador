...
- **`prewarm: true` doubles first-run model load time.** Per WhisperKit's `Configurations.swift:55-66`, prewarm is the load-unload-load Core ML specialization warm-up. Trade-off accepted to keep peak memory bounded on 8 GB Apple Silicon. Revisit if base-model load on M1 8GB exceeds ~60 s.
- **WhisperKit version pin.** `Package.swift` declares `from: "0.9.0"` but `Package.resolved` resolves to `0.18.0`. API stable across that range; capture so future bisects know the tested upper bound.
